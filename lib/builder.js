...
BuildPaths( config, outPath, feedID ) {
    outPath = outPath||config.get('build.dirs.output');
...
function Build( builder, feed, opts ) {
    ...
    this.paths = new BuildPaths( config, opts.buildPath, feed.id );
    this.db = builder.db;
    this.time = new Date();
}
...
Builder.prototype.buildFeed = function( feed, opts ) {
    ...
    var build = new Build( this, feed, opts.buildPath );
    ...
    .then(function prepareBuildDirectory( build ) {
        Log.debug('[%s] Preparing build directory at %s...', build, build.paths.outputRoot );
        // Delete & recreate the build output dir.
        mods.cmds.q.rmdir( build.outputRoot )
        .then(function() {
            return mods.cmds.q.mkdir( build.outputRoot );
        })
        .then(function() {
            return build;
        });
    ...
    .then(function genOutputChecksums( build ) {
        ...
        return files.chksums( build.outputRoot )
        .then(function( files ) {
            build.files = files;
            return build;
        });

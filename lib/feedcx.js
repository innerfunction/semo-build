...
Context.prototype.applyBuildScope = function( obj ) {
    obj.$buildScope = this._lastBuild && this._lastBuild._id;
}
// Test whether an object has the current build scope.
Context.prototype.hasCurrentBuildScope = function( obj ) {
    return obj.$buildScope == this._lastBuild && this._lastBuild._id;
}
...
Context.prototype.record = function( data ) {
    ...
    this.add2q(function() {
        ...
        record = merge( cx._record, data, { '$lastDownload': cx.$lastDownload });
...
Context.prototype.write = function( posts ) {
	...
	this.add2q(function() {
        ...
        var updates = posts._data.map(function( post ) {
            post.$feedID = feedID;
            return function() {
...
// @lastBuild:  The most recent previous build for the feed. Can be null.
// @db:         A db client.
function Context( record, lastBuild, db ) {
    this._opq = [];
    this._record = record;
...
exports.newContext = function( record, lastBuild, db ) {
    return new Context( record, lastBuild, db );
}

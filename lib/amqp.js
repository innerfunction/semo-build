...
function postStatusFilter( message ) {
    return message.status == 'publish' || message.status == 'remove';
}
...
AMQPClient.prototype.addFeed = function( feed ) {
    ...
    Log.debug('Connecting to queue %s...', queueName );
        ...
        ch.consume( queueName, function( message ) {
            ...
            if( postStatusFilter( content ) ) {
                ...
                .fail(function error( e ) {
                    Log.error('Processing post %s for feed %s', content.url, feed.id, e );
                });
            }
        });
    },

...
var Log = require('log4js').getLogger('semo-build/buildcx');
var dust = require('dustjs-q');
...
function resolveFileRefs( refs, find ) {
    ...
    .reduce(function( result, ref ) {
        // If the file ref is a string, and doesn't look like an http URL, or an absolute
        // path (in which case find won't find anything), then treat as a file glob.
        if( typeof ref == 'string' && !URLPattern.test( ref ) && ref[0] != '/' ) {
...
var EmptyFStats = { isDirectory: function() { return false }, notExists: true };
            ...
            .then(function( sourcePath ) {
                ...
                .then(function( stats ) {
                    ...
                    if( sstat.isDirectory() ) {
                        // cp/mv'ing from a directory - can only cp/mv to a directory or a
                        // non-existent file (note that notExists is provided by fstats()
                        // function above.
                        if( !(tstat.isDirectory() || tstat.notExists) ) {
                            var msg = format('Cannot %s directory %s to file %s', op, sourcePath, target._path );
                            throw new Error( msg );
                        }
                        dir = target._path;
                    }
                ...
                .then(function() {
                    ...
                    // If the source's source ends with a dot then copying a directories
...
var ImageOps = {
    ...
    baseURI: function( is, image ) {
        ...
        throw new Error( format('No image reference found on Image object: %s', image.id  ));
    },
...
function newCx( is, find, build ) {

    // The path to a directory containing the build input files.
    var inPath = build.inPath;
    // The path to a directory containing the build output files.
    var outPath = build.paths.outputRoot;
    // Data to do the build with.
    var data = build.data;
    // The feed being built.
    var feed = build.feed;
    // File URI scheme extensions.
    var uriSchemes = feed && feed.exts && feed.exts.uriSchemes||{};
    // The ID of the previous build (if any).
    var prevBuildID = build.prevBuild && build.prevBuild.id;
    ...
    File.prototype.href = am(function( ctx ) {
        ...
        .then(function( path ) {
            ...
            var basePath = semoBasepath( ctx ); // Path to output file if template being eval'd.
            if( basePath ) {
                basePath = mods.path.resolve( outPath, basePath );
                var baseDir = mods.path.dirname( basePath );
                path = mods.path.relative( baseDir, mods.path.resolve( outPath, path ));
            }
            return path;
        });
    });
    // Return a URI for referencing this file. Functions for generating URIs are defined by
    // scheme in the feed's ext.uriSchemes property. A URI generation function should return
    // either a string containing the URI; or a promise resolving to the URI string.
    // Scheme names can be prefixed with '@' (indicating an Eventpac URI reference); the resulting
    // URI will be returned with the same prefix.
    // @scheme: The URI scheme; may include the ':' suffix.
    // @...:    Additional arguments passed to the URI scheme handler.
    File.prototype.uri = am(function() {
        ...
        var uriGen = uriSchemes[scheme];
        if( uriGen ) {
            return q.Q( uriGen.apply( this, arguments ) )
            .then(function( uri ) {
                return prefix+uri;
            });
        }
    ...
    ImageFile.prototype.path = am(function( ctx ) {
        ...
        .then(function( results ) {
            var meta = results[0];
            // URI encode the name to avoid problems with extended chars in filenames.
            var name = utils.removeDiacritics( results[1] );
            // -- HACK --
            var ctx = Object.create( meta );
            ctx.name = mods.path.basename( name, mods.path.extname( name ));
            // -- HACK --
            // Assume the _path is a templated value, resolve against image meta data.
            return dust.renderSource( self._path, ctx ) //meta )
            .then(function( path ) {
    ...
    var cx = {
        // The build (feed) data.
        data: data,
        // Test if an object has the current build scope. (See related methods in feedcx.js).
        hasCurrentBuildScope: function( obj ) {
            return obj.$buildScope == prevBuildID;
        },
...
exports.exec = function( build ) {
    // Resolve image service.
    var imageService = build.config.get('components.images');
    // Find input files.
    return mods.files.search( build.inPath )
    .then(function callBuildFn( find ) {
        // Make build context.
        var cx = newCx( imageService, find, build );
        // Read meta data from previous build.
        var meta = (build.prevBuild && build.prevBuild.meta)||{};
        // Invoke build function.
        return q.Q( build.fn( cx, meta ) )
        .then(function commit( meta ) {
            // Commit build ops.
            return cx.commit()
            .then(function done() {

...
function newCx( is, find, inPath, outPath, data ) {
    ...
    // Return an internal (i.e. Eventpac) URI for referencing this file.
    // The file path is relative to the build output dir.
    // @scheme: The URI scheme; defaults to 'subs'.
    File.prototype.iuri = am(function( scheme ) {
        return this.path()
        .then(function( path ) {
            return format('%s:%s', scheme||'subs', path );
        });
    });
    ...
    var cx = {
        ...
        json: function( data, filename, prettyprint ) {
            ...
            var json = mods.json.stringify( data, undefined, prettyprint ? 4 : undefined );

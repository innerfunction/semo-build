...
function Feed( id, def ) {
    ...
    this.queue = def.queue;         // The feed's AMQP queue of update messages.
...
Installer.prototype.loadFeeds = function( feedModule ) {
    ...
    if( !feeds ) {
        ...
        if( feed.active ) {
            if( feed.queue ) {
                this.startQueuedFeed( feed );
            }
            else if( feed.schedule ) {
                var schedule;
                ...
                try {
                    ...
                    Log.info('Feed %s scheduled, next download at %s', id, feed.job.nextInvocation());
                }
                catch( e ) {
                    Log.error('Scheduling feed %s', id, e );
                }
            }
            else Log.warn("No queue or schedule setting provided for feed '%s'", id );
        }
        else Log.debug('Feed %s is inactive', id );
        // Assign feed to list of feeds.
        this.feeds[id] = feed;
    }
}
// Start a queued feed. Checks whether the feed has a download record, and if not then
// downloads and processes the feed before adding to the queue (to ensure that a complete
// and up-to-date copy of the feed data exists locally).
Installer.prototype.startQueuedFeed = function( feed ) {
    var installer = this;
    function addFeed() {
        Log.debug('Adding queued feed %s...', feed.id );
        installer.components.amqp.addFeed( feed );
    }
    this.db.hasFeedRecord( feed.id )
    .then(function downloadOrStartQueue( hasFeedRecord ) {
        if( hasFeedRecord ) {
            Log.debug('Feed record found for queued feed %s', feed.id );
            addFeed();
        }
        else {
            Log.debug('No feed record found for queued feed %s, processing feed before adding...', feed.id );
            var download = installer.makeFeedProcessFn( feed );
            download().then( addFeed );
        }
    });
}
...
Installer.prototype.makeFeedProcessFn = function( feed ) {
    ...
    return function processFeed() {
        ...
        return downloader.downloadFeed( feed )

...
BuildQueue.prototype.start = function() {
    ...
    .then(function initializeBuildQueue() {
        // Initialize build queue if installer is available. (semob is run without an installer).
        if( bq.installer ) {
...
BuildQueue.prototype.process = function() {
    ...
    if( head && !head.building ) {
        ...
        if( head.time <= now ) {
            ...
            Log.debug('Delaying head item for %d s', (head.time - now) / 1000 );
...
BuildQueue.prototype.buildHeadItem = function() {
    ...
    head.building = true;
    head.sequence = BuildSequence++;
    Log.debug('Building head item [%d]...', head.sequence );
    // Pass the build request to the builder.
    bq.builder.buildFeed( head.feed, head.opts )
    .then(function nextItem() {
        Log.debug('Finished building head item [%d]...', head.sequence );

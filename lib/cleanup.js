 ...
 */
function cleanup( db, feed, keep, publishPath, pretend ) {
...
exports.cli = function() {
    ...
    if( process.argv.length < 3 ) {
        console.log();
        console.log('cleanup [-run] [-keep <count>] [-nofsclean] [-pretend]');
        console.log('Cleanup build records in the database and build results on the file system')
        console.log();
        console.log('\t-run         Perform a cleanup. Nothing is done without this flag');
    ...
    if( run ) {
        ...
        .then(function( components ) {
            ...
            return cleanup( components.db, feed, keep, publishPath, pretend );
        })
        .then(function() {
            console.log('Done');
        })
        ...
        console.log('Nothing done; use -run to perform a cleanup');
    }
}

        ...
        var ops = feeds.map(function makeCleanOp( feedid ) {
            return function cleanOp() {
                ...
                .then(function postsClean() {
                    if( pclean ) {
                        return q.all([ db.getFeedPosts( feedid ), db.getFeedRecord( feedid, false ) ])
                        .spread(function deletePostsAndRecordFromDB( posts, record ) {
...
exports.cli = function() {
    ...
    if( process.argv.length < 3 ) {
        ...
        console.log('  -deepclean   Perform a deep clean. Equivalent to -clean -keep 0 -pclean');

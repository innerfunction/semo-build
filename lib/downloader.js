...
Downloader.prototype.processQueue = function( feedID ) {
    ...
    if( item ) {
        ...
        try {
            ...
            q.all([ db.getFeedRecord( feed.id ), builder.getLastBuildID( feed.id ) ])
            .spread(function( record, lastBuildID ) {
                Log.debug('Creating download context for feed %s: (%s)', feed.id, lastBuildID );
                var _cx = cx.newContext( record, lastBuildID, db );

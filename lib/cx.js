...
function newCx( is, inPath, outPath ) {
    ...
    File.prototype = {
        ...
        // file.
        href: am(function( ctx ) {
            return this.path()
            .then(function( path ) {
                // The output path of the template currently being evaluated.
                var basePath = semoBasepath( ctx );
                return mods.path.relative( basePath, path );
            });
        }),
        path: am(function() {
            return q.Q( this._path );
        }),
        // Resolve the files content.
        content: am(function() {
            return q.all([ q.nfcall( mods.fs.readFile, this.source ), this.mimeType() ])
    ...
    mods.util.inherits( ContentFile, File );
    ContentFile.prototype.content = am(function() {
    ...
    mods.util.inherits( ImageFile, ContentFile );
    ...
    mods.util.inherits( Evald, Content );
    ...
    Evald.prototype.name = am(function( ctx, name ) {
        return dust.render( name||this._name, ctx.push( this._data ) );
    });
    ...
    Evald.prototype.data = am(function( ctx ) {
        ctx = ctx.push( this._data );
        this.template()
        .then(function( template ) {
            return dust.render( template, ctx );
        });
    });
    ...
    mods.util.inherits( Image, Content );
    ...
    //Images.prototype = [];
    mods.util.inherits( Images, Contents );
...
var start = require('./semo/core/start'); // TODO: Temporary import path ref.
...
exports.run = function( inPath, outPath, fn ) {
    ...
    start.startWithComponents(['dao','content','images'], function( ok, config ) {
        ...
        var cx = newCx( is, inPath, outPath );
        try {
            fn( cx );

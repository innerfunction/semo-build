...
Context.prototype.record = function( data ) {
    var cx = this;
    this.add2q(function() {
        var record = merge( cx._record, data, { '.lastDownload': cx.$lastDownload });
        return cx.db.update( record.id, record );
    });
    return this;
}
...
Context.prototype.write = function( posts ) {
    ...
    this.add2q(function() {
        ...
        var updates = posts._data.map(function( post ) {
            post['.feedID'] = feedID;
            return function() {
                return cx.db.update( post.id, post );
            }
        });
...
Context.prototype.clean = function( fn ) {
    ...
    this.add2q(function() {
        ...
        var key = JSON.stringify( cx._record['.feedID'] );
        return cx.db.view('semo-build','feed-posts', { key: key, include_docs: 'true' })
        .then(function( data ) {
            // TODO: check format of result data
            var deletes = [];
            ...
            if( deletes.length ) {
                return cx.db.bulkUpdate( deletes );
            }
            return q.Q();
        });
    });

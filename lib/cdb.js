...
// DB update queue.
// Updates are performed in sequence, to avoid conflicts and other errors.
function UpdateQueue() {
    // A queue of db updates.
    this.queue = [];
}
UpdateQueue.prototype.add = function() {
    var dp = Q.defer();
    // If the queue is empty then immediately exec the update;
    // otherwise add to the end of the queue.
    if( this.queue.length == 0 ) {
        this.exec( db, arguments );
    }
    else {
        this.queue.push({ dp: dp, args: arguments });
    }
    return dp.promise;
}
UpdateQueue.prototype.exec = function( dp, args ) {
    var self = this;
    // Invoke the db update. 'args' will be the db op followed by its arguments, e.g. 'db.update, doc._id, doc'
    q.nfcall.apply( q, args )
    ...
    .fin(function fin() {
        // If the queue has items then exec the next item.
        if( self.queue.length > 0 ) {
            var next = queue.shift();
            self.exec( next.dp, next.args );
        }
    });
}
...
function makeCDBClient( db ) {
    ...
    return {
        ...
        create: function( doc ) {
            ...
            return updateQueue.add( db.create, doc );
        },
        update: function( doc ) {
            //return q.nfcall( db.update, doc._id, doc );
            return updateQueue.add( db.update, doc._id, doc );
        },
        bulkUpdate: function( docs ) {
            //return q.nfcall( db.bulkUpdate, docs );
            return updateQueue.add( db.bulkUpdate, docs );
        },
        remove: function( doc ) {
            //return q.nfcall( db.remove, doc._id );
            return updateQueue.add( db.remove, doc._id );
        },
